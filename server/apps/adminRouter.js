...
adminRouter.get("/courses/:courseId", async (req, res) => {
  try {
    if (req.params.courseId) {
      ...
      const lessonId = lessonResult.data.map((id) => id.lesson_id);
      const lessonAndSubResult = await supabase
        .from("lessons")
        .select("*,sub_lessons(*)")
        .in("lesson_id", lessonId)
        .order("priority", { ascending: true });
      // .order("sub_lessons.priority", { ascending: true });
      ...
      if (
        ...
        return res.json({
          data: {
            ...
            lessons: lessonAndSubResult.data,
          },
        });
      }
    } else {
